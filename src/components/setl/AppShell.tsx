...
const BAR_H = "calc(52px + env(safe-area-inset-top, 0px))";
...
export function AppShell({
  ...
  return (
    <>
      {/* Fixed app bar — one element, height = 52px + safe-area */}
      <header
      ...
      {/* Single spacer offset for the fixed bar */}
      <div style={{ paddingTop: BAR_H }}>{children}</div>
    </>
  );
}
...
function NavDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const onStart = (e: TouchEvent) => { startX = e.touches[0].clientX; currentX = startX; };
    const onMove = (e: TouchEvent) => { currentX = e.touches[0].clientX; };
    const onEnd = () => {
      if (startX !== null && currentX !== null && startX - currentX > 60) onClose();
  ...
  return (
    <>
      ...
      <aside
        ...
        aria-hidden={!open}
        className="fixed inset-y-0 left-0 flex w-[280px] flex-col transition-transform duration-200 ease-out"
        style={{
          zIndex: 70,
      ...
      >
        ...
        <Link
          ...
          style={{ minHeight: 48, color: isEvents ? "var(--magenta)" : DRAWER_FG, fontWeight: isEvents ? 600 : 500 }}
        >
          {isEvents && <ActiveBar />}
          Events
        </Link>
        <Link
          ...
          onClick={onClose}
          className="relative flex items-center px-4 text-[16px]"
          style={{ minHeight: 48, color: isVendors ? "var(--magenta)" : DRAWER_FG, fontWeight: isVendors ? 600 : 500 }}
        >
          {isVendors && <ActiveBar />}
          Vendors
        </Link>
        ...
        <div
          ...
          style={{ color: "rgba(250,248,249,0.5)", letterSpacing: "0.12em" }}
        >
          Events
        </div>
          ...
          {EVENTS.map((e) => {
            ...
            return (
              <Link
                ...
                style={{ minHeight: 48, color: active ? "var(--magenta)" : DRAWER_FG, fontWeight: active ? 600 : 500 }}
              >
                {active && <ActiveBar />}
                <span className="truncate">{e.name}</span>
                <span
                  className="num text-[13px] tabular-nums"
                  style={{ color: active ? "var(--magenta)" : "rgba(250,248,249,0.7)" }}
                >
                  {fmt(np)}
                </span>
              </Link>
...
function ActiveBar() {
  return (
    <span aria-hidden className="absolute left-0 top-0 bottom-0 w-[3px]" style={{ backgroundColor: "var(--magenta)" }} />
  );
}
